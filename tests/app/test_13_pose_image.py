...
def test_pose_from_config(app_base_path, config):
    """
    pose 어플리케이션을 image input 으로 실행하고 결과를 검증
    ...
    cfg = config('app')['pose_image'] # Load cfg_app.yaml >> refer to tests/app/conftest.py
    ...
    try:
        ...
        if result_file.exists():
            ...
            result_file.rename(f"output/pose_{cfg.get('expected_result')}")
        ...
        result_image_path = f"output/pose_{cfg.get('expected_result')}"
