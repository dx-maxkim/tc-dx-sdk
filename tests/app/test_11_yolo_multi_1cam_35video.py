...
def test_yolo_from_config(app_base_path, timeout_sec, config):
    """
    yolo_multi 어플리케이션을 파일에 정의된 configuration 로 수행 후 결과 검증 (1 cam + 35-ch videos)
    - Pass: 문제없이 수행되고 지정된 시간까지 실행 후 에러없이 종료
    - Fail: 동작을 안하거나 동작 간 에러 발생
    """
    # YAML 파일에서 설정 정보를 불러옵니다.
    cfg = config('app')['yolo_multi_1cam_35video']
    ...
    # test case 에서 정의한 1 cam + 35-ch video script 를 copy
    if copy_config == 'true':
        src_file = "utils/yolo_multi_1cam_35_demo.json"
        dst_file = f"{app_base_path}/example/yolo_multi/yolo_multi_1cam_35_demo.json"
