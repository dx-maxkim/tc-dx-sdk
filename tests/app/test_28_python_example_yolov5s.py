...
def load_config():
    ...
    return config['python_yolov5s_test']

@pytest.mark.smoke
@pytest.mark.normal
@pytest.mark.stress
def test_run_yolov5s_from_config(app_base_path):
    """
    ...
    if not command_str or not expected_output or not target_venv_python:
        pytest.fail("config_28.yaml 파일에 'command' or 'expected_output' or 'target_venv_python' 키가 없습니다.")
