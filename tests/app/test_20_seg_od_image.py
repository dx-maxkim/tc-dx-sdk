...
def test_seg_od_from_config(app_base_path, config):
    ...
    cfg = config('app')['od_segmentation_image'] # Load 'cfg_app.yaml' from 'tests/app/conftest.py'
    ...
    try:
        ...
        if result_file.exists():
            ...
            result_file.rename(f"output/seg_od_{cfg.get('expected_result')}")
        ...
        result_image_path = f"output/seg_od_{cfg.get('expected_result')}"
